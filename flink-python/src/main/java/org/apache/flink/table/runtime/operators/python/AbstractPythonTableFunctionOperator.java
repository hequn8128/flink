...
public abstract class AbstractPythonTableFunctionOperator<IN, OUT, UDTFIN, UDTFOUT>
	...
	/**
	 * The Python {@link TableFunction}s to be executed.
	 */
	...
	AbstractPythonTableFunctionOperator(
		PythonFunctionInfo tableFunctions,
		RowType inputType,
		RowType outputType,
		int[] udtfInputOffsets) {
		this.tableFunction = Preconditions.checkNotNull(tableFunctions);
		this.udtfInputOffsets = udtfInputOffsets;
	...
	public void open() throws Exception {
		...
		List<RowType.RowField> outputRowFields = new LinkedList<>(
			outputType.getFields().subList(inputType.getFieldCount(), outputType.getFieldCount()));
		outputRowFields.add(new RowType.RowField("startFlag", new BigIntType(false)));
		udtfOutputType = new RowType(outputRowFields);
		super.open();
	}
	...
	public PythonFunctionRunner<IN> createPythonFunctionRunner() {
		...
		return new ProjectUdtfInputPythonScalarFunctionRunner(createPythonFunctionRunner(udtfResultReceiver));
	}
	...
	public abstract PythonFunctionRunner<UDTFIN> createPythonFunctionRunner(FnDataReceiver<UDTFOUT> resultReceiver);

	// todo: remove this class
	private class ProjectUdtfInputPythonScalarFunctionRunner implements PythonFunctionRunner<IN> {

		private final PythonFunctionRunner<UDTFIN> pythonFunctionRunner;

		ProjectUdtfInputPythonScalarFunctionRunner(PythonFunctionRunner<UDTFIN> pythonFunctionRunner) {
