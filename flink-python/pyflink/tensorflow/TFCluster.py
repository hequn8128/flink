...
class InputMode(object):
    ...
    FLINK = 1  #: Spark is responsible for feeding data to the TensorFlow application via an RDD.
...
class TFCluster(object):
    ...
    def train(self, dataRDD, num_epochs=0, feed_timeout=600, qname='input'):
        ...
        if isinstance(dataRDD, DStream):
            # Spark Streaming
            dataRDD.foreachRDD(lambda rdd: rdd.foreachPartition(TFFlinkNode.train(self.cluster_info, self.cluster_meta, feed_timeout=feed_timeout, qname=qname)))
        else:
            # Spark RDD
            ...
            unionRDD.foreachPartition(TFFlinkNode.train(self.cluster_info, self.cluster_meta, feed_timeout=feed_timeout, qname=qname))
    ...
    def inference(self, dataRDD, feed_timeout=600, qname='input'):
        ...
        assert qname in self.queues, "Unknown queue: {}".format(qname)
        return dataRDD.mapPartitions(TFFlinkNode.inference(self.cluster_info, feed_timeout=feed_timeout, qname=qname))
    ...
    def shutdown(self, ssc=None, grace_secs=0, timeout=259200):
        ...
        workerRDD.foreachPartition(TFFlinkNode.shutdown(self.cluster_info, grace_secs, self.queues))
...
def run(map_fun, tf_args, num_executors, num_ps, tensorboard=False, input_mode=InputMode.TENSORFLOW,
    ...
    """
    logger.info("Reserving TFFlinkNodes {0}".format("w/ TensorBoard" if tensorboard else ""))
    ...
    logger.info("Waiting for TFFlinkNodes to start")
    cluster_info = server.await_reservations(tf_status, reservation_timeout)
    logger.info("All TFFlinkNodes started")
