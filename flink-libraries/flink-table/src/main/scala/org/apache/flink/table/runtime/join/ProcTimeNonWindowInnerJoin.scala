...
class ProcTimeNonWindowInnerJoin(
  ...
  // tmp list to store left expired records
  private var leftRemoveList: ListState[Row] = _
  // tmp list to store right expired rtecords
  private var rightRemoveList: ListState[Row] = _
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    // initialize left and right remove list state
    val leftRemoveListDescriptor = new ListStateDescriptor[Row]("leftRemoveList", leftType)
    val rightRemoveListDescriptor = new ListStateDescriptor[Row]("rightRemoveList", rightType)
    leftRemoveList = getRuntimeContext.getListState(leftRemoveListDescriptor)
    rightRemoveList = getRuntimeContext.getListState(rightRemoveListDescriptor)
  ...
  override def processElement1(
    ...
    commonFlatMap(valueC, ctx, out, timerState1, leftState, rightState, rightRemoveList, true)
  }
  ...
  override def processElement2(
    ...
    commonFlatMap(valueC, ctx, out, timerState2, rightState, leftState, leftRemoveList, false)
  }
  ...
  def commonFlatMap(
      ...
      oppositeSideState: MapState[Row, JTuple2[Int, Long]],
      removeListState: ListState[Row],
      isLeft: Boolean): Unit = {
    ...
    val oppositeSideRowsIterator = oppositeSideState.keys().iterator()
    // join opposite side data
    while (oppositeSideRowsIterator.hasNext) {
      val oppositeRow = oppositeSideRowsIterator.next()
      val oppositeCntAndExpiredTime = oppositeSideState.get(oppositeRow)
      // if opposite record is expired
      if (stateCleaningEnabled && curProcessTime >= oppositeCntAndExpiredTime.f1) {
        removeListState.add(oppositeRow)
      }
      // if opposite record is valid
      else {
        cRowWrapper.setTimes(oppositeCntAndExpiredTime.f0)
        if (isLeft) {
          joiner.join(value.row, oppositeRow, cRowWrapper)
        } else {
          joiner.join(oppositeRow, value.row, cRowWrapper)
        }
      }
    }

    // remove rows which are expired
    val removeListIterator = removeListState.get().iterator()
    while (removeListIterator.hasNext) {
      oppositeSideState.remove(removeListIterator.next())
    }
    removeListState.clear()
  }
  ...
  private def expireOutTimeRow(
    ...
    val removeListIterator = removeListState.get().iterator()
    while (removeListIterator.hasNext && validTimestamp) {
      rowMapState.remove(removeListIterator.next())
    }
    removeListState.clear()
