...
class DataStreamLastRowRule
  ...
  override def onMatch(call: RelOptRuleCall): Unit = {
    ...
    val uniqueKeyNames = scan.getTable.unwrap(classOf[UpsertStreamTable[_]]).uniqueKeys
    val uniqueKeyIndex = scan.getRowType.getFieldNames.zipWithIndex
    ...
    val dataStreamLastRow = new DataStreamLastRow(
      scan.getCluster, traitSet, upsertStreamScan, inputSchema, inputSchema, uniqueKeyIndex)
