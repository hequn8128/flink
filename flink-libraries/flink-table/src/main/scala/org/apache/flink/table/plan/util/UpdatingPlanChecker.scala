...
object UpdatingPlanChecker {
  ...
  private class UniqueKeyExtractor {
    ...
    // belong to the same group, i.e., pk. A node can have keys if it generates the keys by
    // itself or it forwards keys from its input(s).
    ...
    def getOutputKeysForJoin(
        inNames: Seq[String],
        inputKeys: List[(String, String)],
