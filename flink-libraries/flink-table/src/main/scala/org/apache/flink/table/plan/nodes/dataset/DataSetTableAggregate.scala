...
import org.apache.flink.table.runtime.aggregate.{AggregateUtil, DataSetPreTableAggFunction, DataSetTableAggFunction, DataSetFinalTableAggFunction}
...
class DataSetTableAggregate(
  ...
  override def computeSelfCost(planner: RelOptPlanner, metadata: RelMetadataQuery): RelOptCost = {
    ...
    // rowCnt * 2 to estimate output rowcnt
    planner.getCostFactory.makeCost(rowCnt * 2, rowCnt * aggCnt, rowCnt * rowSize)
  }
  ...
  override def translateToPlan(
    ...
    val rowSchema = new RowSchema(rowRelDataType)

    val rowTypeInfo = FlinkTypeFactory.toInternalRowTypeInfo(getRowType).asInstanceOf[RowTypeInfo]

    val (
      preAgg: Option[DataSetPreTableAggFunction],
      preAggType: Option[TypeInformation[Row]],
      finalAgg: Either[DataSetTableAggFunction, DataSetFinalTableAggFunction]
      ) = AggregateUtil.createDataSetTableAggregateFunctions(
        tableEnv.getConfig,
        false,
        inputDS.getType,
        None,
    ...
    if (grouping.length > 0) {
      ...
      // global aggregation
      val aggOpName = s"flatAggregate:($aggString)"
