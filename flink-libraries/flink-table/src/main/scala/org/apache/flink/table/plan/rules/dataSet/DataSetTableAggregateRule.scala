...
class DataSetTableAggregateRule
  extends ConverterRule(
    classOf[FlinkLogicalTableAggregate],
    FlinkConventions.LOGICAL,
    FlinkConventions.DATASET,
    "DataSetTableAggregateRule") {

  override def matches(call: RelOptRuleCall): Boolean = {
    val agg: FlinkLogicalTableAggregate = call.rel(0).asInstanceOf[FlinkLogicalTableAggregate]
...
object DataSetTableAggregateRule {
  val INSTANCE = new DataSetTableAggregateRule
}
