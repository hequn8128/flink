...
class TableAggregateWindowValidationTest extends TableTestBase {
  ...
  def testInvalidWindowProperty(): Unit = {
    ...
    table
      ...
      .select('_1, '_2.start) // property in non windowed table
  }
  ...
  def testTumbleUdAggWithInvalidArgs(): Unit = {
    ...
    table
      ...
      .groupBy('string, 'w)
      .flatAggregate(top3(0, 1L))
      // udaf can not be used in the select of flatAggregate
      .select('string, weightedAvg('string, 'int))
  }
  ...
  def testInvalidWindowPropertyOnRowCountsTumblingWindow(): Unit = {
    ...
    .groupBy('string, 'w)
    .flatAggregate(top3(0, 1L))
    .select('string, '_1, 'w.start, 'w.end) // invalid start/end on rows-count window
  }
