...
class TableAggregateHarnessTest extends HarnessTestBase {
  ...
  def testTableAggregateWithoutEmitRetract(): Unit = {
    ...
    val resultTable = source.groupBy('a)
      ...
      .select('_1 as 'catgory, '_2 as 'v, '_3 as 'rank)

    val ds = tEnv.optimize(resultTable.getRelNode, false)
      .asInstanceOf[DataStreamRel]
      .translateToPlan(tEnv, queryConfig)

    val trans = extractExpectedTransformation(ds.getTransformation, "TableAggregate")

    val testHarness =
      createHarnessTester(
        trans.getOperator.asInstanceOf[OneInputStreamOperator[Any, Any]],
        trans.getStateKeySelector.asInstanceOf[KeySelector[Any, Any]],
        trans.getStateKeyType.asInstanceOf[TypeInformation[Any]])
