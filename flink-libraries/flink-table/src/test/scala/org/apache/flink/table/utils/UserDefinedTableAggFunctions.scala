...
class TopN(n: Int) extends TableAggregateFunction[(Int, Long, Int), TopNAccum] {
  ...
  def emitValue(acc: TopNAccum, out: RetractableCollector[(Int, Long, Int)]): Unit = {
    emitValue(acc, out, false)
  }

  def emitValueWithRetract(acc: TopNAccum, out: RetractableCollector[(Int, Long, Int)]): Unit = {
    emitValue(acc, out, true)
  }

  def emitValue(
    acc: TopNAccum,
    out: RetractableCollector[(Int, Long, Int)],
    ...
    for (input <- acc.input) {
      ...
      if (acc.dataSize == n && acc.min >= input._2) {
        ...
        if (acc.dataSize == 0) {
          ...
          acc.min = input._2
          out.collect((input._1, input._2, 0))
        } else {
...
/**
  ...
  */
class Top3WithRetractInput extends TableAggregateFunction[(Int, Long, Int), Top3Wrapper] {
  ...
  def emitValueWithRetract(acc: Top3Wrapper, out: RetractableCollector[(Int, Long, Int)]): Unit = {
    emitValue(acc, out, true)
  }
  ...
  def emitValue(
    ...
    while (i < minLength && i < 3) {
      if (oldTop(i) != newTop(i)) {
        if (isRetract) {
          out.retract((category, oldTop(i), i))
        }
        out.collect((category, newTop(i), i))
      }
      i += 1
    }
