 ...
 * functions {@link org.apache.flink.table.functions.AggregateFunction}.
 *
 * <p>A {@link MapView} can be backed by a Java {@link HashMap} or a state backend, depending on
 * the context in which the aggregation function is used.
 *
 * <p>At runtime {@link MapView} will be replaced by a
 * <code>org.apache.flink.table.dataview.StateMapView</code> if it is backed by a state backend.
 *
...
public class MapView<K, V> implements DataView {
	 ...
	 */
	public MapView(TypeInformation<K> keyTypeInfo, TypeInformation<V> valueTypeInfo) {
		this(keyTypeInfo, valueTypeInfo, new HashMap<>());
	}
	...
	/**
	 * Return the value for the specified key or <code>null</code> if the key is not in the map view.
	 *
	 ...
	 * @return <code>true</code> if there exists a value for the given key, false otherwise.
	 * @throws Exception Thrown if the system cannot access the map.
	 */
	...
	public boolean equals(Object other) {
		...
		return (this.map == null && mapView.map == null) ||
			this.map.equals(mapView.map);
	}
