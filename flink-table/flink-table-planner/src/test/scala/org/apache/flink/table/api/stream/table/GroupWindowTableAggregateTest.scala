...
class GroupWindowTableAggregateTest extends TableTestBase {
  ...
  @Test
  def testWindowTableAggregateWithIntResultType(): Unit = {
    val func = new EmptyTableAggFuncWithIntResultType
    val table = util.addTable[(Long, Int, Long, Long)]('f0, 'f1, 'f2, 'f3.rowtime, 'f4.proctime)

    val windowedTable = table
      .window(Session withGap 3.milli on 'f3 as 'w)
      .groupBy('w, 'f0)
      ...
      unaryNode(
        ...
        unaryNode(
          ...
          unaryNode(
            "DataStreamCalc",
            streamTableNode(table),
            term("select", "f0", "f1", "f3")
          ),
          term("groupBy", "f0"),
          term("window", "SessionGroupWindow('w, 'f3, 3.millis)"),
