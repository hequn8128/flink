...
import org.apache.flink.table.expressions.{TableAggFunctionCall, _}
...
case class TableAggregate(
  ...
  override def output: Seq[Attribute] = {
    ...
    if (tableAggFunctionCall.alias.nonEmpty) {
      generatedNames = tableAggFunctionCall.alias.toList.toArray
    }
  ...
  override protected[logical] def construct(relBuilder: RelBuilder): RelBuilder = {
    child.construct(relBuilder)
    relBuilder.asInstanceOf[FlinkRelBuilder].tableAggregate(
      relBuilder.groupKey(groupingExpressions.map(_.toRexNode(relBuilder)).asJava),
