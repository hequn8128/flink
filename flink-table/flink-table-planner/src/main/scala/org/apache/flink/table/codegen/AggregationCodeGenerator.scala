...
class AggregationCodeGenerator(
  ...
  def generateTableAggregations(
    ...
    val emitRetractValue = "emitRetractValueIncrementally"
    ...
    // Set emitValue as the default emit method here and set it to emitRetractValueIncrementally on
    // condition that: 1. emitRetractValueIncrementally has been defined in the table aggregate
    // function and 2. the operator supports emit incrementally, for example, window flatAggregate
    // doesn't support emit incrementally)
      ...
      aggregates.zipWithIndex.map {
        case (a, i) =>
          if (supportEmitIncrementally) {
            try {
              finalEmitMethodName = emitRetractValue
              getUserDefinedMethod(
                a,
                emitRetractValue,
              ...
              case _: ValidationException =>
                ...
                // emitRetractValueIncrementally method
