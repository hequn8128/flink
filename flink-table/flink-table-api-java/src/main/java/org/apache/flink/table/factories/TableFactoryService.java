...
public class TableFactoryService {
	...
	private static final transient Logger LOG = LoggerFactory.getLogger(TableFactoryService.class);
	...
	private static <T> T filterBySupportedProperties(
		...
		AtomicReference<Optional<String>> lastKey = new AtomicReference<>(Optional.empty());
		List<TableFactory> supportedFactories = classFactories.stream().filter(factory -> {
			...
			// ignore context keys
			Stream<String> givenContextFreeKeys = plainGivenKeys.stream().filter(p -> !requiredContextKeys.contains(p));
			Stream<String> givenFilteredKeys = filterSupportedPropertiesFactorySpecific(
				factory,
				givenContextFreeKeys);

			return givenFilteredKeys.allMatch(k -> {
				lastKey.set(Optional.of(k));
				return tuple2.f0.contains(k) || tuple2.f1.stream().anyMatch(p -> k.startsWith(p));
			});
		}).collect(Collectors.toList());

		if (supportedFactories.isEmpty() && classFactories.size() == 1 && lastKey.get().isPresent()) {
			// special case: when there is only one matching factory but the last property key
			// was incorrect
			TableFactory factory = classFactories.get(0);
			Tuple2<List<String>, List<String>> tuple2 = normalizeSupportedProperties(factory);

			String errorMessage = "The matching factory '" + factory.getClass().getName() + "' doesn't support '" + lastKey.get().get() + "'.\n" +
				"\n" +
	 ...
	 */
	private static Stream<String> filterSupportedPropertiesFactorySpecific(
